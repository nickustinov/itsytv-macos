...
def main():
    ...
    for rel in releases:
        ...
        for a in assets:
            ...
            size = a.get('size', 0)
            # include common macOS packaging types
            if not any(name.endswith(ext) for ext in ('.zip', '.dmg', '.pkg')):
                continue
    ...
    for it in items:
        ...
        xml.append(f'<enclosure url="{sax.escape(it["url"])}" sparkle:version="{sax.escape(it["version"])}" length="{sax.escape(it["length"])}" type="application/octet-stream" />')
        xml.append('</item>')

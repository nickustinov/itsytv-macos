...
- [x] **Text input** — Remote keyboard input via `_tiStart`/`_tiC` for search fields (requires NSKeyedArchiver encoding)
- [ ] **Session management** — Proper `_sessionStart`/`_sessionStop` lifecycle with `_systemInfo` exchange

## MRP protocol features

- [x] **Media controls** — Play, pause, next/prev track, skip forward/back via MRP commands (more reliable than HID for playback control)
- [x] **Now playing** — Title, artist, album, playback state, position, duration via MRP `SetStateMessage` push updates
- [x] **Artwork** — Album art / video thumbnail via MRP `PlaybackQueueRequestMessage`
- [x] **Mini player UI** — Compact now-playing widget in the menu bar with artwork, metadata, and transport controls
